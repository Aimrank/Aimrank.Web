    ...
    internal class EventMapper : IEventMapper
    {
        public IIntegrationEvent Map(IDomainEvent @event)
            => @event switch
            {
                ...
                {
                    {Match: {Status: MatchStatus.Starting}} => new MatchStartingEvent(e.Match.Id, e.Match.Map, e.Match.Address,
                        e.Match.Players.Select(p => p.UserId.Value),
                        e.Match.Lobbies.Select(l => l.LobbyId.Value)),
                    {Match: {Status: MatchStatus.Started}} => new MatchStartedEvent(e.Match.Id, e.Match.Map, e.Match.Address,
                        e.Match.Players.Select(p => p.UserId.Value),
                        e.Match.Lobbies.Select(l => l.LobbyId.Value)),
                },
